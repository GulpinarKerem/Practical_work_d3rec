...
def generate_popularity_bins(df_inter, item_col="item_id", user_col="user_id",
                             ...
                             bin_ratios=(0.2, 0.6, 0.2)):
    """
    df_inter: preprocessing'te kullanılan interaction dataframe'i
              en az item_id ve user_id içermeli

    Amaç:
    - Her item'ın kaç kez oynandığını (pop) bul
    - Pop'a göre sıralayıp 3 seviyeye böl:
        high → ilk %20
        mid  → sonraki %60
        low  → son %20
    """
...
def calculate_user_specific_prefs(df_inter, item_to_cat_map):
    """
    ...
    for uid, group in user_groups:
        ...
        # Ürünlerin kategorilerini bul
        cats = [item_to_cat_map.get(it, 1) for it in items] # default mid (1)
        
        total = len(cats)
        # 0: high, 1: mid, 2: low
        c0 = cats.count(0) / total
        c1 = cats.count(1) / total
        c2 = cats.count(2) / total
        
        user_prefs[uid] = [c0, c1, c2]
...
class PreProcess():
    def __init__(self, args, dir_path, use_cache=False):
        ...
        if use_cache and os.path.exists(clean_dataset_split_path):
            ...
            if "pop_bin" in df_clean.columns:
                df_clean = df_clean.drop(columns=["pop_bin"])
    ...
    def clean_and_sort(self, df, drop_num, drop_rating, le_user, le_item):
        ...
        if drop_num:
            ...
            print(f"Min user interactions after core_setting: {user_counts.min()}")
            print(f"Min item interactions after core_setting: {item_counts.min()}")
        ...
        print(df[[self.str_item, "pop_bin"]].head())
        df[self.str_cate] = df["pop_bin"].apply(lambda x: [x])

        print("Category column (cate) is now based on pop_bin. Example:")
        print(df[[self.str_item, "pop_bin", self.str_cate]].head())
    ...
    def split_group_by_user(self, df, ratio, str_cols):
        ...
        for user_id, df_user in tqdm(group_users):
            ...
            num_test = np.ceil(num_items * test_ratio).astype(int)  # Follow sklearn split policy
...
# preprocessing.py içindeki ilgili fonksiyonun en sağlam hali bu olsun:
def get_user_gender_dict(dataset_dir_path):
    import os
    # Denenecek yollar listesi
    possible_paths = [
        os.path.join(dataset_dir_path, 'users.dat'), # ml-1m klasörü içi
        os.path.join(os.path.dirname(dataset_dir_path), 'users.dat'), # dataset klasörü içi
        os.path.join(os.getcwd(), 'users.dat') # ana çalışma dizini
    ]
    ...
    if users_file:
        ...
        print(f"--- CRITICAL ERROR: users.dat NOT FOUND in any of these: {possible_paths}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--drop_num', default=5, type=int, help="5-core filtering")
