...
def main(args, dataset_dir_path, best_model_path):
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if epoch - best_epoch >= args.patient:
            print('-' * 18)
        ...
        if epoch % args.eval_freq == 0:
            ...
            val_recall = valid_results[2][1]

            print(
                f'Evaluation validation recall@20: {val_recall:.4f}, '
                f'time: {str(timedelta(seconds=int(time.time() - start)))}'
            )
            ...
            if val_recall > best_recall:
                ...
                test_results = evaluate(
                    ...
                    loader=test_loader,
                    sp_test=sp_test,
                    sp_train_valid=sp_train + sp_valid,
                    topk=args.topK,
                    item_category=dataset.item_category,
                    n_cate=dataset.num_cate,
                    temperature=1.0,
                ...
                print('Evaluation Best test')
                print_metric_results(args.topK, test_results)

    print('#' * 106, f'Training time: {str(timedelta(seconds=int(time.time() - start_total)))}')
    print(f"Test Metric At Best Valid Metric, epoch: {best_epoch}")
    if best_test_result is not None:
        print_metric_results(args.topK, best_test_result)
    else:
        print("No best test result recorded.")


def get_args_parser():
    parser = argparse.ArgumentParser(description="D3Rec Phase 6 Training", add_help=True)
    ...
    parser.add_argument('--topK', default=[10, 20], type=int, nargs='+', help="Top-k values")
    parser.add_argument('--patient', default=1000, type=int, help="Early stopping patience")
    ...
    parser.add_argument('--split_ratio', default=[6, 2, 2], type=int, nargs="+", help="Train/Valid/Test split")
    parser.add_argument('--drop_num', default=5, type=int, help="Minimum interactions")
    parser.add_argument('--drop_rating', default=4, type=int, help="Minimum explicit rating to keep")
    parser.add_argument('--test_w_valid', action='store_true', help="Test with train+valid history as input")

    ##### Model hyperparameters #####
    parser.add_argument('--dims', type=int, default=[600, 200], nargs="+")
    parser.add_argument('--dropout', default=0.5, type=float)
    parser.add_argument('--dim_step', default=10, type=int)
    parser.add_argument('--lamda', default=10.0, type=float)
    parser.add_argument('--w_max', default=10.0, type=float)
