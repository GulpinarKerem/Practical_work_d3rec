...
def main(args, dataset_dir_path, best_model_path, user_gender_map):
    ...
    for temperature in [0.1, 0.2, 0.5, 1.0, 5.0]:
        start = time.time()
        evaluate_with_gender(
            args, model, diffusion, loader, sp_test,
            dataset, temperature, user_gender_map
        )
        print(f'Evaluation with w: {args.guide_w}, temperature {temperature} '
...
def evaluate_with_gender(args, model, diffusion, loader, sp_test, dataset, temperature, user_gender_map):
    ...
    n_cate = dataset.num_cate
    num_items = dataset.num_items  # <-- gerçek catalog coverage için lazım
    ...
    with torch.no_grad():
        for x_0, prob, prob_pred in loader:
            ...
            # low-pop yönlendirilmiş öneriler
    ...
    def print_res(name, data):
        if not data['target']:
            return
        # num_items veriyoruz -> CatalogCoverage gerçek değeri gösterir
        m = compute_metric(
            data['target'], data['pred'],
            args.topK, item_category, n_cate,
            num_items=num_items
        )
        print(f"\n>>> {name}:")
...
def get_args_parser():
    ...
    parser.add_argument('--str_cols', type=str, nargs='+',
